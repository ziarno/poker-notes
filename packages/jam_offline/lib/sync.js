...
async function sync() {
  const { autoSync, archive, handleSyncErrors } = Offline.config;
  if (!autoSync) {
    return;
  }
  ...
  const offlineCollectionNamesToReconcile = [];
  for (const [name, { filter }] of offlineCollections.entries()) {
    if (name !== archive?.name || !isEmpty(filter)) offlineCollectionNamesToReconcile.push(name);
  }
  ...
  return Tracker.autorun(async computation => {
    ...
    for (const { id, name, args: mArgs, collectionName, docId } of methods) {
      ...
      try {
        ...
        if (docId) {
          // if we have a docId, it means that it was added while offline. if it was added with a preset _id, then we re-use the _id and avoid removing the doc
          const _id = stringifyId(result.insertedId ?? result); // used to support Mongo.ObjectID
          if (_id !== docId) {
            await removeOfflineDoc(
              localCollections[collectionName],
              docId
            ).catch(error => console.error('removeOfflineDoc error', error)); // we want to remove the doc now that its been successfully replayed and inserted on the server. we don't want these errors to be included in methodErrors.
            await updateMethods(methods, docId, _id).catch(error =>
              console.error(error => 'updateMethods error', error)
            ); // we'll want to update any changes made to that document with the real _id after it's inserted on the server. this assumes that the _id is returned whenever a document is inserted which is fairly standard. we don't want these errors to be included in methodErrors.
          }
        }
      } catch (error) {
    ...
    if (offlineCollectionNamesToReconcile.length) {
      pauseObservers(offlineCollectionNamesToReconcile);
      ...
      for (const { name, removeIds, syncedAt } of synced) {
        ...
        await updateSyncs(name, syncedAt).catch(error =>
          errors.push({ name, error })
        );
      }

      resumeObservers(offlineCollectionNamesToReconcile);
