const config = {
  cache: {
    // Note: html, js, and css caching is handled automatically
    ...
    (async function () {
      await Promise.all([
        ...
        caches
          ...
          .then(cache =>
            cache.addAll(
              config.cache.include.map(p =>
                getVersion(p) ? p : `${p}?v=${config.cache.version}`
              )
            )
          ),
      ])
      ...
      await Promise.all(
        keys.map(async cached => {
          ...
          const asset = config.cache.include.find(
            key =>
              stripQuery(key) === pathname &&
              (getVersion(key) || String(config.cache.version)) !==
                getVersion(cached.url)
          )
...
self.addEventListener('fetch', event => {
  ...
  if (
    ...
    [...config.cache.exclude, METEOR_WEBSOCKET].some(path =>
  ...
  if (
    dev &&
    request.method === 'POST' &&
    request.url.includes('/__meteor__/dynamic-import/fetch')
  ) {
    // only used when in development, in prod meteor handles this
...
function hasHash(element) {
  return (
    ...
    /[?&](hash|meteor_css_resource|meteor_js_resource)=/.test(element)
  )
}
function isMeteorResource(query) {
  return /[?&](meteor_css_resource|meteor_js_resource)=/.test(query)
}
function stripQuery(element) {
  return typeof element === 'string' ? element.split('?')[0] : ''
}
...
function hasSameHash(first, second) {
  ...
  if (isMeteorResource(firstQuery)) {
    // prod will use meteor resource
    return firstPath === secondPath
  }

  return (
    getQueryParam(firstQuery, 'hash') === getQueryParam(secondQuery, 'hash')
  )
}

function isSame(first, second) {
  return isMeteorResource(first)
    ? getExtension(first) === getExtension(second)
    : stripQuery(first) === stripQuery(second)
}
//
...
async function handleRequest(request) {
  ...
  const precache =
    pathname !== HTML_KEY &&
    config.cache.include.find(item => item.includes(pathname))
  const version = precache
    ? getVersion(precache) || String(config.cache.version)
    : !isHashed && getVersion(request.url)
  ...
  if (cached) {
    ...
    const notTextHtml =
      !isHashed && !REGEX.texthtml.test(cached.headers.get('content-type'))
  ...
  try {
    ...
    const cacheName = REGEX.html.test(contentType)
      ? PAGES
      : isHashed
        ? BUNDLES
        : ASSETS
    ...
    return (
      cached ||
      new Response('No connection to the server', {
        status: 503,
...
async function handleDynamicImportRequest(request) {
  ...
  try {
    ...
    return (
      cached ||
      new Response('No cached dynamic import available', {
        status: 503,
        statusText: 'Service Unavailable',
        headers: { 'Content-Type': 'text/plain' },
      })
